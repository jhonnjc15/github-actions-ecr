...
output "sfn_role_arn" {
  value = data.aws_iam_role.sfn_role.arn
}

output "scheduler_role_arn" {
  value = data.aws_iam_role.scheduler_role.arn
}

output "schedule_name" {
  description = "Nombre del scheduler (null si schedule_enabled=false)"
  value       = try(aws_scheduler_schedule.schedule[0].name, null)
}

output "schedule_arn" {
  description = "ARN del scheduler (null si schedule_enabled=false)"
